...
import { RGFSBnftContract, blockExplorer } from "@/consts/parameters";
import { NFTCard } from "@/components/NFTCard";


export default function RGFSBGallery() {
const nftsPerPage = 20;
const [page, setPage] = useState(1);
const [search, setSearch] = useState<string>("");
const debouncedSearchTerm = useDebounce(search, 500);
const { data: nfts, isLoading } = useReadContract(getNFTs, {
contract: RGFSBnftContract,
count: nftsPerPage,
...
{ <a href={blockExplorer+"/address/"+RGFSBnftContract.address} className = "pointer" target={"_blank"}>{contractMetadata.name}</a>}
</h1>
...
<NFTCard nft={nft} nftName = {"RGFSB"} key={nft.id.toString()} />
) : null}
