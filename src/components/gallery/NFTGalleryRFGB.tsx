...
import { RFGBnftContract, blockExplorer } from "@/consts/parameters";
import { NFTCard } from "@/components/NFTCard";


export default function RFGBGallery() {
const nftsPerPage = 20;
const [page, setPage] = useState(1);
const [search, setSearch] = useState<string>("");
const debouncedSearchTerm = useDebounce(search, 500);
const { data: nfts, isLoading } = useReadContract(getNFTs, {
contract: RFGBnftContract,
count: nftsPerPage,
...
{ <a href={blockExplorer+"/address/"+RFGBnftContract.address} className = "pointer" target={"_blank"}>{contractMetadata.name}</a>}
</h1>
<h2 className="text-xl font-bold text-white">
{String(contractMetadata.description)}
</h2>
</div>
) : contractLoading ? (
...
<NFTCard nft={nft} nftName = {"RFGB"} key={nft.id.toString()} />
) : null}
