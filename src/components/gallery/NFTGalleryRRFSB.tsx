...
import { RRFSBnftContract, blockExplorer } from "@/consts/parameters";
import { NFTCard } from "@/components/NFTCard";


export default function RRFSBGallery() {
const nftsPerPage = 20;
const [page, setPage] = useState(1);
const [search, setSearch] = useState<string>("");
const debouncedSearchTerm = useDebounce(search, 500);
const { data: nfts, isLoading } = useReadContract(getNFTs, {
contract: RRFSBnftContract,
count: nftsPerPage,
...
{ <a href={blockExplorer+"/address/"+RRFSBnftContract.address} className = "pointer" target={"_blank"}>{contractMetadata.name}</a>}
</h1>
...
<NFTCard nft={nft} nftName = {"RRFSB"} key={nft.id.toString()} />
) : null}
